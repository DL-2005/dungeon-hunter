...
# --- Attack telegraphs -----------------------------------------------------
# Called by ai/tasks/attack_player.gd and ai/tasks/ranged_attack.gd instead of
# dealing damage the instant the cooldown clears. This gives a brief, visible
# wind-up so a dodge can actually be timed to the attack instead of being
# pure luck. Same interface on MobAI.gd so the shared task scripts work for
# both bosses and mobs.
...
func resolve_melee_attack() -> void:
	if player != null and player.has_method("take_damage") and player.health > 0.0 and is_alive():
		...
		print("Boss hits player for ", attack_damage, " damage")
...
func resolve_ranged_attack() -> void:
	if player != null and player.has_method("take_damage") and player.health > 0.0 and is_alive():
		...
		print("Boss ranged-hits player for ", ranged_damage, " damage")
...
func reset_and_respawn(exclude_indices: Array[int] = [], reroll_preset: bool = true) -> void:
	...
	is_telegraphing_melee = false
	is_telegraphing_ranged = false
	_melee_telegraph_timer = 0.0
	_ranged_telegraph_timer = 0.0
	var spr := _get_visual_sprite()
	if spr != null:
		spr.modulate = Color(1, 1, 1)
